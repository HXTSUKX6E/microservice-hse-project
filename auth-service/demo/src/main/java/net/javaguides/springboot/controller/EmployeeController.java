...
public class EmployeeController {
    ...
    @GetMapping("/auth/emp/confirm-email-change")
    @Async
    public CompletableFuture<ResponseEntity<Map<String, Object>>> confirmEmailChange(@RequestParam String token) {
        ...
        Optional<Employee> employeeOptional = employeeRepositrory.findByPendingLogin(newEmail);
        if (employeeOptional.isPresent()) {
            Employee employee = employeeOptional.get();
            employee.setLogin(newEmail);  // Обновляем основной login
            employee.setPendingLogin(null);  // Очищаем временное поле
            employeeRepositrory.save(employee);
